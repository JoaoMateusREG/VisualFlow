...
type CustomNodeType = Node<CustomNodeData>;
  ...
  // Mapeamento de tipos de nós para seus componentes visuais
...
// Opções padrão para as conexões (arestas) entre nós
...
// Gerador de IDs únicos para novos nós
let nodeId = 0;
...
const updateNodeIdCounter = (nodes: CustomNodeType[]) => {
   ...
   */
  const App: React.FC = () => {
    // Estado do ReactFlow (nós e conexões)
    const [nodes, setNodes, onNodesChange] = useNodesState<CustomNodeData>([]);
    const [edges, setEdges, onEdgesChange] = useEdgesState([]);
    
    // Estado de seleção e interface
    ...
    // Hook personalizado para gerenciar execuções remotas (API)
    const execution = useExecution();

    // Callback disparado ao conectar dois nós
    ...
    /**
     * Atualiza os dados de um nó específico quando o usuário edita no ConfigPanel
     */
    ...
    /**
     * Gerencia o drop de novos nós ou upload de arquivos JSON na área do fluxo
     */
    const onDrop = useCallback(
      (event: React.DragEvent) => {
        event.preventDefault();

        // Verificar se é um arquivo (importação de workflow JSON)
          ...
          if (file.type === 'application/json' || file.name.endsWith('.json')) {
            ...
            reader.onload = (e) => {
              try {
                ...
                // Tenta carregar de diferentes formatos de workflow
                let nodesData = null;
                let edgesData = null;
                let name = 'Workflow Importado';
                
                // Formato 1: rawData direto (exportado da app)
                ...
                // Formato 2: SavedWorkflow do backend
        ...
        // Lidar com drop de novo nó da sidebar
    ...
    /**
     * Executa o fluxo APENAS gerando o código no console (Modo Execução Local/Debug)
     */
    ...
    /**
     * Envia o fluxo para o BACKEND executar remotamente
     */
    const onExecuteRemote = useCallback(async () => {
      ...
      // Executar no backend (API)
      await execution.executeFlow(flowData);
    }, [nodes, edges, execution, workflowName]);

    /**
     * Salva o fluxo atual como um arquivo JSON localmente
     */
    ...
    /**
     * Carrega um workflow salvo anteriormente via upload de arquivo JSON
     */
        ...
        if (file) {
          ...
          reader.onload = (e) => {
            try {
              ...
              // Tentar carregar de diferentes formatos de workflow
              let nodesData = null;
              let edgesData = null;
              let name = 'Workflow Importado';
    ...
    /**
     * Carrega um workflow vindo do WorkflowManager (DB)
     */
